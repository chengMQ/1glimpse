...
def list2html(table_html):
    ...
    head = "<div sitename='{}'><table style='table-layout:fixed;' cellspacing=5 cellpadding=7 width='100%' align='center'><tbody><p><h2 align='center'>{}</h2></p>blablablabla</tbody></table></div>".format(
        website,website)
    if not list1:
...
def withpics_to_file(ss):
    ...
    with open('./pages/%s[cover-yes].html' % this_day, 'w', encoding='utf-8') as f:
        ...
        scode = '<meta charset="utf-8"><meta name=”viewport” content=”target-densitydpi=device-dpi” /><title>一瞥日报 %s</title><style> body{background-color:#a4a4a4} </style><style>A {text-decoration: NONE} </style><div align="right">采集时间：%s</div><hr>' % (this_time,this_time) + '<br>'.join(
            ss)
        f.write(scode)
...
def refresh_index():
    ...
    str1 = '<!--网址列表--start--><p style="font-size:18px;">最后更新：{}</p><p style="font-size:18px;"><strong>当前订阅列表：</strong>{}</p><div align="center"><table style="width:100%;" border="0" cellpadding="2" cellspacing="0" align="center"><tbody><tr><td width="9999"><strong style="font-size:35px;">有图版</strong></td><td width="50%"><strong style="font-size:35px;">无图版</strong></td></tr><tr><td><ul>{}</ul></td><td><ul>{}</ul></td></tr></tbody></table></div><!--网址列表--end-->'.format(this_time,titles,list1,list2)
    # print(str1)
