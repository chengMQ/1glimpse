...
def withpics_to_file(ss):
    ...
    with open('./pages/%s[cover-yes-mobile].html' % this_day, 'w', encoding='utf-8') as f:
        scode=re.sub('<td.*?>|</td>','',scode).replace('font-size:16px;', 'font-size:46px;').replace('font-size:18px;', 'font-size:58px;')
        f.write(scode)
...
def withoutpic_to_file(ss):
    ...
    with open('./pages/%s[cover-no-mobile].html' % this_day, 'w', encoding='utf-8') as f:
        scode=re.sub('<td.*?>|</td>|<img.*?>|<video.*?</video>','',scode).replace('font-size:16px;', 'font-size:46px;').replace('font-size:18px;', 'font-size:58px;')
        f.write(scode)
...
def refresh_index():
    ...
    str1 = '<!--网址列表--start--><p style="font-size:18px;">最后更新：{}</p><p style="font-size:18px;"><strong>当前订阅列表：</strong>{}</p><div align="center"><table style="width:100%;" border="0" cellpadding="2" cellspacing="0" align="center"><tbody><tr><td width="25%"><strong style="font-size:20px;">手机-有图版</strong></td><td width="25%"><strong style="font-size:20px;">手机-无图版</strong></td><td width="9999"><strong style="font-size:20px;">PC-有图版</strong></td><td width="25%"><strong style="font-size:20px;">PC-无图版</strong></td></tr><tr><td><ul>{}</ul></td><td><ul>{}</ul></td><td><ul>{}</ul></td><td><ul>{}</ul></td></tr></tbody></table></div><!--网址列表--end-->'.format(
...
def refresh_old():
    ...
    list4 = ''.join([i.replace('[cover-no-mobile]blabla', '')
                     for i in list0 if '[cover-no-mobile]' in i])
    str1 = '<meta charset="utf-8"><!--网址列表--start--><div align="center"><table style="width:100%;" border="0" cellpadding="2" cellspacing="0" align="center"><tbody><tr><td width="25%"><strong style="font-size:20px;">手机-有图版</strong></td><td width="25%"><strong style="font-size:20px;">手机-无图版</strong></td><td width="9999"><strong style="font-size:20px;">PC-有图版</strong></td><td width="25%"><strong style="font-size:20px;">PC-无图版</strong></td></tr><tr><td><ul>{}</ul></td><td><ul>{}</ul></td><td><ul>{}</ul></td><td><ul>{}</ul></td></tr></tbody></table></div><!--网址列表--end-->'.format(
...
if __name__ == '__main__':
    ...
    titles2 = ' | '.join([re.sub('target="_blank" href=".*?"','href="#%s"'%re.sub('<.*?>','',i[0]),i[0]) for i in ss])
    withpics_to_file(ss)
